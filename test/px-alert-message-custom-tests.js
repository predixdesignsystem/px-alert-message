...
describe("Alert action buttons update correctly", function() {
  ...
  it('action button reflects the "dismiss" property', function(done) {
    ...
    flush(function() {
      var icon = Polymer.dom(alertMessageEl.root).querySelector(
        ".action .dismiss > px-icon"
      );
...
describe("Alert Dismiss", function() {
  ...
  it("`visible` is set to false when the alert message is dismissed", function(done) {
    var dismissAlert = fixture("PxDismissAlertMessage");
    flush(function() {
      var closeBtn = Polymer.dom(dismissAlert.root).querySelector(
        "#dismissButton"
      );
      closeBtn.click();
      expect(dismissAlert.visible).to.equal(false);
  ...
  it("alert message is hidden after px-alert-message-hidden event is fired when dismiss X is tapped", function(done) {
    ...
    var handler = closeBtnAlert.addEventListener(
      "px-alert-message-hidden",
      function(evt) {
        expect(
          Polymer.dom(closeBtnAlert.root)
            .querySelector("#alert")
            .classList.contains("alert-message--visible")
        ).equal(false);
        assert.equal(
          evt.type,
          "px-alert-message-hidden",
          "event name should be px-alert-message-hidden"
        );
  ...
  it("alert message is hidden after px-alert-message-hidden event is fired when OK button is tapped", function(done) {
    ...
    var handler = okBtnAlert.addEventListener(
      "px-alert-message-hidden",
      function(evt) {
        expect(
          Polymer.dom(okBtnAlert.root)
            .querySelector("#alert")
            .classList.contains("alert-message--visible")
        ).equal(false);
        assert.equal(
...
describe("Truncation shortens message", function(done) {
  ...
  it('show more" btn is visible when the alert title and message are too long and do not fit', function(done) {
    ...
    var moreBtn = Polymer.dom(longTitleAlert.root).querySelector(
      "#showMoreButton"
    );
    flush(function() {
  ...
  it(" if message has a max height of around 32px", function() {
    ...
    assert.closeTo(
      parseInt(
        window.getComputedStyle(MessageDiv, null).getPropertyValue("max-height")
      ),
      32,
      3,
      "they are close"
  ...
  it("truncates longMessageAlert properly", function(done) {
    ...
    var MessageDiv = Polymer.dom(longMessageAlert.root).querySelector(
      "#message"
    );
    flush(function() {
      expect(MessageDiv.scrollHeight >= MessageDiv.clientHeight).equal(true);
...
describe("Truncated message", function() {
  ...
  it("opens the message when the `expanded` property is set in the component", function() {
    var longAlert = fixture("PxLongTitleAlertExpanded");
    var message = Polymer.dom(longAlert.root).querySelector("#message");
    var moreBtn = Polymer.dom(longAlert.root).querySelector("#showMoreButton");
    longAlert.expanded = true;
    expect(message.classList.contains("collapsed-message")).to.equal(false);
    expect(moreBtn.classList.contains("visuallyhidden")).to.equal(true);
  });
